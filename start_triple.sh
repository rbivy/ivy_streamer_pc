#!/bin/bash
# Start triple streamer with proper cleanup
...
sshpass -p "$PI_PASSWORD" ssh -o StrictHostKeyChecking=no "$PI_USER@$PI_IP" \
    "pkill -f triple_streamer.py" 2>/dev/null || true
...
if [ "$ports_ready" = false ]; then
    ...
    print_info "Try: ./ssh_pi_robust.sh \"sudo fuser -k 5000/tcp 5001/tcp 5002/tcp\""
    exit 1
fi

# Start new streamer
print_info "Starting triple streamer..."
echo ""
...
then
    STREAMER_PID=$!
    print_status "Triple streamer started successfully (background PID: $STREAMER_PID)"
    ...
    for port in 5000 5001 5002; do
        if nc -z -w5 "$PI_IP" $port 2>/dev/null; then
            print_status "Port $port: RGB/Left/Right stream ready"
        else
            print_error "Port $port: Stream not ready"
    ...
    if [ "$all_ports_ready" = true ]; then
        print_status "All streams are ready!"
        echo ""
        ...
        echo "  ./test_triple_advanced_overlay.sh  # Advanced overlays (recommended)"
        echo "  ./test_triple.sh                   # Simple receivers"
        echo ""
        print_info "To stop the streamer: ./ssh_pi_robust.sh \"pkill -f triple_streamer.py\""
    else
    ...
    print_error "Failed to start triple streamer"
    exit 1
fi